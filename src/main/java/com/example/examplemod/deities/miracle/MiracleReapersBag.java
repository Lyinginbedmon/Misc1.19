...
public class MiracleReapersBag extends Miracle 
{
	...
	public void onPlayerDeath(LivingDropsEvent event)
	{
		if(!event.isCanceled() && event.getEntity().getType() == EntityType.PLAYER && !event.getDrops().isEmpty())
		{
			...
			world.setBlock(bagPos, ExBlocks.REAPER_BAG.get().getStateForPlacement(new BlockPlaceContext(world, null, InteractionHand.MAIN_HAND, ItemStack.EMPTY, new BlockHitResult(Vec3.ZERO, Direction.DOWN, bagPos, false))), 0);
